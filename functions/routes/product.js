...
const defaultDescription = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum nec ante sit amet tellus mattis dignissim ut at diam. Curabitur ac elementum sapien. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas."
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const rawReviews = await Review.where("productId", "==", rawProduct.id).get()
        const reviews =  rawReviews.docs.map(doc => {return {...doc.data(), id: doc.id}})
...
router.post("/create", authToken, async (req, res) => {
    const { name, price, image, tags, description = defaultDescription, stock = 11} = JSON.parse(req.header("product"))
    const isStaff = req.user.isStaff
